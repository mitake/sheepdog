...
static pthread_mutex_t jfile_lock = PTHREAD_MUTEX_INITIALIZER;
...
static pthread_mutex_t journal_commit_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void journal_commit_data_work(struct work *work)
{
	...
	pthread_mutex_unlock(&journal_commit_mutex);
}
...
static void switch_journal_file(void)
{
	...
	if (pthread_mutex_trylock(&journal_commit_mutex) == EBUSY) {
		sd_err("journal file in commiting, you might need"
		       " enlarge jfile size");
		pthread_mutex_lock(&journal_commit_mutex);
	}
...
static int journal_file_write(struct journal_descriptor *jd, const char *buf)
{
	...
	pthread_mutex_lock(&jfile_lock);
	if (!jfile_enough_space(wsize))
		switch_journal_file();
	woff = jfile.pos;
	jfile.pos += wsize;
	pthread_mutex_unlock(&jfile_lock);
