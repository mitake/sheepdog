			    ...
			    const struct node_id *owner)
{
	...
	for (int i = 0; i < entry->nr_participants; i++) {
		if (memcmp(&entry->participants[i], owner, sizeof(*owner))) {
			idx = i;
			break;
		}
	}
...
worker_fn bool is_refresh_required(uint32_t vid)
{
	...
	for (int i = 0; i < entry->nr_participants; i++) {
		if (memcmp(&entry->participants[i], &sys->this_node.nid,
			   sizeof(sys->this_node.nid)))
			continue;
				   ...
				   const struct node_id *sender)
{
	...
	if (validate) {
		...
		for (int i = 0; i < entry->nr_participants; i++) {
			if (memcmp(&entry->participants[i], sender,
				   sizeof(*sender)))
				entry->participants_state[i] =
					SHARED_LOCK_STATE_INVALIDATED;
			else {
